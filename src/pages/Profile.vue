...
const canSavePass = computed(() => {
  if (
    ...
    passwordData.confirm_password?.length
  ) {
    return true
  } else return false
})
...
const saveInfo = async () => {
  const response = await appStore.patchData(
    '/api/secure-storage/user/' + appStore.user_id + '/',
    userData
  )
  if (response) {
    canEdit.value = false
...
const changePassword = async () => {
  await appStore.postData('/api/change-password/', passwordData)
}
...
<template>
  <div class="flex justify-between">
    ...
    <div class="flex flex-col items-end">
      <div
        class="text-[32px] font-medium text-white bg-[#C37B17] rounded-lg py-3 w-[72px] h-[72px] text-center uppercase"
      >
        {{ initials }}
      </div>
      <v-btn
